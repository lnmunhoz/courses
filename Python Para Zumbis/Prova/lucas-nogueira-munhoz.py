...
# retorna True se a palavra for uma preposição Goglon
def isGoglon(s):
	return s[0] in 'zmb' and s[-1] not in 'zmb'

# retorna True se a palavra é um verbo Goglon
def isVerbo(s):
	return len(s) == 7 and s[-1] not in 'zmb'

# retorna True se a palavra é um verbo Goglon e está em primeira pessoa
def isVerboPP(s):
	return isVerbo(s) and s[0] not in 'zmb'

# cria um dicionário a partir de duas strings do mesmo tamanho
def make_dict(s1, s2):
	if len(s1) != len(s2): return Exception("len(s1) != len(s2)")
	new_dict = dict()
	for i in range(len(s1)):
		new_dict[s1[i]] = s2[i]
	return new_dict

# traduz uma palavra, a partir de um respectivo dicionário
def translate(palavra, dicionario):
	result = ''
	for letra in palavra:
		result += dicionario[letra]
	return result

# Testes txtA questão D
def goglon_sorter(txt):
	# cria os dicionários para os respectivos vocabulários.
	dicionario_comum = make_dict('zmbtshjpnwlrcxkqvdgf', 'abcdefghijklmnopqrst')
	dicionario_goglon= make_dict('abcdefghijklmnopqrst', 'zmbtshjpnwlrcxkqvdgf')

	# traduz palavras para o dicionario comum
	lista_comum = []
	for palavra_goglon in txt:
		palavra_comum = translate(palavra_goglon, dicionario_comum)
		lista_comum.append(palavra_comum)

	# ordena palavras lexicograficamente
	lista_comum.sort()

	# traduz palavra para o dicionario goglon
	lista_goglon = []
	for palavra_comum in lista_comum:
		palavra_goglon = translate(palavra_comum, dicionario_goglon)
		lista_goglon.append(palavra_goglon)

	# retorna lista com palavra goglon ordenadas lexicograficamente
	return lista_goglon

def numero_goglon(palavra_goglon):
	vocabulario = 'zmbtshjpnwlrcxkqvdgf'
	dicionario_numeros = dict()
	for i in range(len(vocabulario)):
		dicionario_numeros[vocabulario[i]] = i

	numero = 0
	for i in range(len(palavra_goglon)):
		numero += dicionario_numeros[palavra_goglon[i]] * (20 ** i)
...
def questao_C(txt):
	verbosPP = 0
	for palavra in txt:
		if isVerboPP(palavra):
			verbosPP += 1
	return verbosPP

def questao_D(txt):
	return ' '.join(goglon_sorter(txt))	

def questao_E(txt):
	quantidade_magicos = 0
	numeros_magicos = []
	for palavra in txt:
		if numero_goglon(palavra) % 42 == 0 and digitos_unicos(palavra):
			quantidade_magicos += 1
			numeros_magicos.append(palavra)
	return '%d %s' %(quantidade_magicos, ' '.join(numeros_magicos))


print('A;%d' %questao_A(txtB))
print('B;%d' %questao_B(txtB))
print('C;%d' %questao_C(txtB))
print('D;%s' %questao_D(txtB))
print('E;%s' %questao_E(txtB))
