...
def lone_sum(a, b, c):
  l = [a, b ,c]
  return sum(l) if len(l) == len(set(l)) else sum(x for x in l if x not in [y for y in l if l.count(y) > 1])
...
def count_code(s):
  ...
  try:
    ...
    s_parcial = s
    while i < len(s):
      s_parcial = s_parcial[i:]
      index = s_parcial.index('co')
      code = s_parcial[index : index + 4]
      if code[2] in string.ascii_lowercase and code[3] == 'e':
        count_code += 1
      i = index + 3
    return count_code
  except:
