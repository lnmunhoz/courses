...
def first_last6(nums):
  return nums[0] is 6 or nums[-1] is 6
...
def squirrel_play(temp, is_summer):
  max = 90 if not is_summer else 100
  return temp >= 60 and temp <= max
...
def pego_correndo(speed, is_birthday):
  soma_cinco = 5 if is_birthday else 0
  return 0 if speed <= 60 + soma_cinco else 1 if speed <= 80 + soma_cinco else 2
...
def alarm_clock(day, vacation):
  semana = [x for x in range(1, 6)]
  fim_semana = [0, 6]
  return 'off' if vacation and day in fim_semana else '7:00' if not vacation and day in semana else '10:00'
